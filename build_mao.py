#!/usr/bin/env python3
"""
Build a static HTML site from Mao Zedong's Obsidian vault.
Usage: python3 build_mao.py
"""
...
.type-选集精选{background:#7C5E2A}
.type-诗词{background:#8B2F2F}
.type-核心思想{background:#5E4A8B}
.type-工作方法{background:#2A6B4F}
.type-人物{background:#3A5E8B}
.type-事件{background:#4A6E2A}
...
.section:nth-child(5){animation-delay:.85s}
...
@media(max-width:768px){
  ...
  .stats-row{padding:0 16px;margin:0 0 20px;grid-template-columns:repeat(3,1fr)}
...
def build_sidebar_html(files, current_stem=""):
    """Build navy sidebar."""
    groups = {cat: [] for cat in CATEGORY_DIRS}
    for f in files:
        cat = f["category"]
        if cat in groups:
            groups[cat].append(f)
    ...
    # Sort
    for cat in ["selected-works", "poems", "events"]:
        groups[cat].sort(key=lambda x: x["stem"])
    for cat in ["concepts", "methods", "people", "index-pages"]:
        groups[cat].sort(key=lambda x: x["stem"])
...
def build_backlinks_html(stem, backlinks_map, link_map):
    """Build the right-sidebar backlinks panel."""
...
def build_homepage(files, link_map, ref_counts):
    ...
    poem_files = [f for f in files if f["category"] == "poems"]
    work_files = [f for f in files if f["category"] == "selected-works"]
    people_files = [f for f in files if f["category"] == "people"]
    event_files = [f for f in files if f["category"] == "events"]

    total_links = sum(ref_counts.values())
    arrow = "→"

    # People data for avatars
    people_data = {
        "周恩来":   ("周", "毛泽东最亲密的战友"),
        "朱德":     ("朱", "朱毛红军，军事搭档"),
        "刘少奇":   ("刘", "白区工作代表"),
...
<section class="hero-section">
  <div class="hero-eyebrow">毛泽东 · 思想与实践</div>
  <h1 class="hero-title">毛泽东<span class="gold">知识库</span></h1>
  <p class="hero-sub"><b>{len(work_files)} 篇</b>选集精选，<b>{len(poem_files)} 首</b>诗词，<b>{len(concept_files)} 个</b>核心思想，<b>{len(method_files)} 个</b>工作方法<br>
  从《湖南农民运动考察报告》到《关于正确处理人民内部矛盾的问题》——追踪毛泽东思想的演变轨迹</p>
</section>
...
<div class="section">
  ...
  <p style="font-size:14px;color:var(--text2);margin:-8px 0 16px;font-family:var(--serif)">每张方法卡都附带"你能用上吗？"的实操建议——从矛盾分析法到群众路线，这些方法论至今仍有生命力。</p>
  <div class="tag-cloud">
'''
...
<div class="footer-promo">
  <div class="promo-story">
    <h3>关于本站</h3>
    <p>把毛泽东散落在《毛选》、诗词、会议讲话、外交会见中的思想，整理成一张可以漫游的知识图谱。从 1925 年《中国社会各阶级的分析》到 1957 年《关于正确处理人民内部矛盾的问题》，跨越 32 年。</p>
    <p>独有视角：<strong>核心思想追踪</strong>（矛盾论 / 实践论 / 群众路线 / 持久战）和 <strong>10 个工作方法</strong>——每张方法卡都附带"你能用上吗？"的实操建议。</p>
    <p class="promo-credit">本站是由作者与 <strong>Claude Code</strong> 共同完成的。</p>
  </div>
</div>
...
def main():
    ...
    # Copy static assets
    assets_src = Path(__file__).parent / "assets-mao"
    ...
    for f in files:
        ...
        if cat == "home":
            ...
            html = wrap_page(title, body_html, files, current_stem=stem, right_html=bl_html)
